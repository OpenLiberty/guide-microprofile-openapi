...
Add OpenAPI annotations [hotspot=APIResponse file=0]`@APIResponse`, [hotspot=Operation file=0]`@Operation` and [hotspot=Parameter file=0]`@Parameter` to the two JAX-RS methods, [hotspot=host-property hotspot=Parameter file=0]`getPropertiesForHost()` and [hotspot=listContents file=0]`listContents()`.
...
[role='edit_command_text']
Add OpenAPI annotations `@Schema` to the [hotspot=InventoryList file=1]`InventoryList` class and the [hotspot=Systems file=1]`systems` variable.
...
[role='edit_command_text']
Add OpenAPI annotations `@Schema` to the [hotspot=SystemData file=2]`SystemData` class, the [hotspot=Hostname file=2]`hostname` variable and the [hotspot=Properties file=2]`properties` variable.
