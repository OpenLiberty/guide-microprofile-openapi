...
Add OpenAPI annotations to the two JAX-RS methods, [hotspot=host-property hotspot=Parameter file=0]`getPropertiesForHost()` and [hotspot=listContents file=0]`listContents()`.
...
document doesn't have a very meaningful description of the [hotspot=InventoryList file=1]`InventoryList` POJO and hence it's very
difficult to tell exactly what that POJO is used for. To describe the [hotspot=InventoryList file=1]`InventoryList` POJO in more detail, augment the
...
[role='edit_command_text']
Add OpenAPI annotations to the [hotspot=InventoryList file=1]`InventoryList` class and the [hotspot=Systems file=1]`systems` variable.
...
[role='edit_command_text']
Add OpenAPI annotations to the [hotspot=SystemData file=2]`SystemData` class, the [hotspot=Hostname file=2]`hostname` variable and the [hotspot=Properties file=2]`properties` variable.
...
Since this document is complete, you can also set the [hotspot file=0]`mp.openapi.scan.disable` property
to `true` in the [hotspot file=1]`src/main/webapp/META-INF/microprofile-config.properties` file.
