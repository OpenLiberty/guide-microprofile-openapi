...
The warning and error messages are expected and result from a request to a bad or an unknown host name. This request is made in the test [hotspot=testUnknownHost file=0]`testUnknownHost()` from [hotspot file=0]`InventoryEndpointIT`.
