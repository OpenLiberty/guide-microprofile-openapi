...
Add OpenAPI annotations to the two JAX-RS methods, [hotspot=host-property hotspot=Paramter file=0]`getPropertiesForHost()` and [hotspot=listContents file=0]`listContents()`.
...
[source, text, linenums, role='code_column tags=config']
----
