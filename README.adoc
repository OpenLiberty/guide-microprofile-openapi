...
some of the other MicroProfile guides, you may recognize this service. While the
...
The first block of code sets the `info` element of the OpenAPI tree to be the `Info` object created
using the `OASFactory`. The second block of code is similar and it sets the `servers` element to be a
...
https://github.com/eclipse/microprofile-open-api/blob/master/tck/src/main/java/org/eclipse/microprofile/openapi/reader/MyOASModelReaderImpl.java
...
example from the previous section. Use that code in this method to set the missing `info` and `servers`
