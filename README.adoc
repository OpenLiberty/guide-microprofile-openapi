...
Before you proceed, note that the 1.0 version of the MicroProfile OpenAPI specification does not define how the `/openapi` endpoint may be partitioned in the event of multiple JAX-RS applications running on the same Liberty instance. In other words, you must stick to one JAX-RS application per Liberty instance as the behaviour for handling multiple applications is currently undefined.
...
ifndef::cloud-hosted[]
Since the Open Liberty instance was started in dev mode at the beginning of the guide, your changes were automatically picked up. Refresh the {openapi-url}[{openapi-url}^] URL to see the updated OpenAPI tree. The two endpoints at which your JAX-RS endpoint methods are served are now more meaningful:
endif::[]

ifdef::cloud-hosted[]
Since the Open Liberty instance was started in dev mode at the beginning of the guide, your changes were automatically picked up. Run the following curl command to see the updated OpenAPI tree:
```bash
curl http://localhost:9080/openapi
```
