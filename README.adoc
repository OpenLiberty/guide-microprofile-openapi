...
| [hotspot=32-42]`@APIResponses` | A container for multiple responses from an API operation. This annotation is
optional, but it can be helpful to organize a method with multiple responses.
| [hotspot=34-37]`@APIResponse`  | Describes a single response from an API operation.
| [hotspot=41]`@Content`      | Provides a schema and examples for a particular media type.
| [hotspot=42]`@Schema`       | Defines the input and output data types.
| [hotspot=43-46]`@Operation`    | Describes a single API operation on a path.
| [hotspot=48-52]`@Parameter`    | Describes a single operation parameter.
|===
