...
package. Note that this is the only element which cannot be removed. Note also that this method is
always called last for a given filter. Hence, make sure that it doesn't override any other filter
...
can be disabled using the `mp.openapi.scan.disable` property. If the document is partially complete,
then it can be augmented with code or filters.
