...
You might want to update or remove certain elements and fields of the OpenAPI document once its been
generated. All of this can be done using the `OASFilter` interface.
...
In this case, you are matching the `404` response returned by the `/inventory/systems/{hostname}`
endpoint and filling in its the previously missing description. To remove a `APIResponse` element,
...
do not test the correctness of the generated OpenAPIn document. If a test failure occurs, then you might
