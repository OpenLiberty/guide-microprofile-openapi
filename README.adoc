...
You will learn how to expose and filter RESTful APIs from annotations, POJOs, and/or static OpenAPI
files using MicroProfile OpenAPI.

The OpenAPI specification (previously known as the Swagger specification) defines a standard interface
for exposing RESTful APIs. This gives both humans and computers a better understanding of the functionalities of
services without requiring direct access to their source code or documentation. The MicroProfile OpenAPI
...
some of the other MicroProfile guides, you may recognize this service. While the application still
functions the same way, the `system` service has been merged into the `inventory` service due a requirement
of MicroProfile OpenAPI that states "The 1.0 version of the MicroProfile OpenAPI specification does
not define how the `/openapi` endpoint may be partitioned in the event that the MicroProfile runtime
supports deployment of multiple applications". As a result, the `properties` resource of the `system`
service is now served at `inventory/properties` rather than `system/properties`.
...
You may also add OpenAPI annotations to POJOs. Augment the `src/main/java/io/openliberty/guides/inventory/model/InventoryList.java`
...
Implement `OASFilter` in the `src/main/java/io/openliberty/guides/inventory/filter/InventoryOASFilter.java` file:
...
The `filterOpenAPI()` method allows filtering of the singleton `OpenAPI` element. Notice that your
current OpenAPI document doesn't provide much information on the application itself, as well as what
server and port it runs on (`info` and `servers` elements). You can use the `OASFactory` class to
manually set these elements, and in general any elements of the OpenAPI tree from the `org.eclipse.microprofile.openapi.models`
package. Note, the `OpenAPI` element is the only element which cannot be removed since that would mean
removing the whole OpenAPI tree. Note also that the `filterOpenAPI()` method is always called
last for a given filter. Hence, make sure that it doesn't override any other filter operations that
...
To learn more about what elements you can filter, visit the official MicroProfile OpenAPI GitHub repository:
...
OpenAPI document. Every such document is required to be named `openapi` with an extension of `yml`, `yaml`, or `json` and
be provided under the `META-INF` directory. If more than one document is found that matches one of these
extensions, then the behaviour for choosing this file is undefined. Depending on the scenario, the document
may be fully complete or partially complete. If the document is fully complete, then annotation scanning
can be disabled using the `mp.openapi.scan.disable` MicroProfile Config property. If the document is
partially complete, then it can be augmented with code or filters.
...
missing APIs for the `/inventory/properties` endpoint. Open this document in your favourite text
editor and uncomment all of its lines. Since this is a complete OpenAPI document, you
no longer need to have any of the OpenAPI annotations you've defined previously. You don't have to
remove these explicitly, instead simply set the `mp.openapi.scan.disable` property to `true` in the
`src/main/webapp/META-INF/microprofile-config.properties` file:
...
the `mvn install` command. You can also run these tests separately from the build using the `mvn verify`
